...
class InputKeyboard extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      value: this.convertNextValue(exists(props.value) ? props.value : '')
    }
  }
  handleChange(e) {
  ...
  convertNextValue(value) {
    value = String(value).replace(/,/g, "");
    value = value.indexOf('0') !== -1 ? value : value || '';
    value = this.props.type === 'number' && value !== '' ? parseInt(value) : value;
    value = this.props.commas ? numberWithCommas(value): value;
    return value
  }

  setNextValue(value) {
    var nextValue = this.convertNextValue(value);
