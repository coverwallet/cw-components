...
class InputKeyboard extends React.Component {
  ...
  render() {
    const { min, currency, currencyType = 'dollar', commas, width, type, autoFocus, maxLength, negatives, showMobileNumpad } = this.props;
    ...
    return (
      <div className="input-keyboard" style={{ width }}>
        ...
        <input
          ref="input"
          type={(commas || showMobileNumpad) ? 'text' : type}
          className={inputClass}
          pattern={(commas || type !== 'number') && !showMobileNumpad ? '[0-9,-]*' : '[0-9]*'}
